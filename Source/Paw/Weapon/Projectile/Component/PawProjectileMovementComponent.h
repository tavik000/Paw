...
#include "PawProjectileMovementComponent.generated.h"
...
struct FAsyncSweepData
{
	...
	int32 SweepCount;
	int32 HitCount;
	float SubTickTimeRemaining; // After hit, the remain time to process the next sub-tick
...
/**
 * This component handles the movement of projectiles in the game base on UProjectileMovementComponent but with asynchronous sweeps for penetration handling and not use USceneComponent->MoveComponent() function.
 */
UCLASS(ClassGroup=Movement, meta=(BlueprintSpawnableComponent), ShowCategories=(Velocity))
class PAW_API UPawProjectileMovementComponent : public UMovementComponent
{
	GENERATED_BODY()
...
public: // Blueprint-Callable API
	...
	void StartSimulating(const FVector& InitialVelocity);
...
protected: // Protected Helper Methods
	...
	{
		Deflect,
		/** Assume velocity has been deflected, and trigger HandleDeflection(). This is the default return value of HandleBlockingHit(). */
		AdvanceNextSubstep,
		...
		Abort,
		/** Abort all further simulation. Typically used when components have been invalidated or simulation should stop. */
	};
